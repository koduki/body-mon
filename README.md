...
歩数・距離・活動消費・睡眠・心拍・基礎代謝・活動強度はHealth Connectの集計APIを使い、生レコードを単純加算しません。体重と体脂肪率は10分以内の測定だけを組み合わせます。除脂肪量はデータ元による定義差を避けるため、常に体重−脂肪量で計算します。
